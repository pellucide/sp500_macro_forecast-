"""
SSRF Test with S&P 500 Returns - Scale=20
...
results = {
    'SSRF': calculate_metrics(model_preds, actual),
    'Naive (0)': calculate_metrics(naive_preds, actual),
    'Random': calculate_metrics(random_preds, actual),
    'Hist. Mean': calculate_metrics(hist_mean_preds, actual),
...
def bootstrap_ci(preds, actual, n_boot=1000, ci=0.95):
    ...
    return lower, upper, real_sharpe
...
perm_p = permutation_test(model_preds, actual)
print(f"Permutation Test (vs random): p={perm_p:.4f} {'***' if perm_p < 0.01 else '**' if perm_p < 0.05 else '*' if perm_p < 0.1 else ''}")

ci_low, ci_high, ss_sharpe = bootstrap_ci(model_preds, actual)
print(f"Bootstrap 95% CI for Sharpe: [{ci_low:.3f}, {ci_high:.3f}] (Sharpe={ss_sharpe:.3f})")

dm_stat, dm_p = diebold_mariano(model_preds, momentum_preds, actual)
print(f"Diebold-Mariano (SSRF vs Momentum): t={dm_stat:.3f}, p={dm_p:.4f} {'***' if dm_p < 0.01 else '**' if dm_p < 0.05 else '*' if dm_p < 0.1 else ''}")

t_stat, t_p = t_test(model_preds, actual)
...
oos_results = {
    'SSRF': calculate_metrics(ssrf_preds, y_test),
...
oos_ci_low, oos_ci_high, oos_sharpe = bootstrap_ci(ssrf_preds, y_test)
print(f"Bootstrap 95% CI for Sharpe: [{oos_ci_low:.3f}, {oos_ci_high:.3f}] (Sharpe={oos_sharpe:.3f})")
...
if ssrf_oos_sharpe > 0 and ssrf_oos_hit > 50:
    if oos_ci_low > 0:
        ...
        print("⚠️ SSRF MARGINALLY PASSES - CI includes negative")
        print(f"   Direction Accuracy: {ssrf_oos_hit:.1f}%")
