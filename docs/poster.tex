...
\usepackage[paperwidth=36in,paperheight=24in,margin=0.8in]{geometry}
...
\newcommand{\postersection}[1]{%
  \vspace{0.5em}
  \noindent
  \colorbox{rhgreen}{\parbox{\dimexpr\columnwidth-2\fboxsep\relax}{\textcolor{rhbg}{\textbf{\LARGE #1}}}}\par
  \vspace{0.3em}
}
...
\begin{tikzpicture}[remember picture, overlay]
  \fill[rhgreen] (current page.north west) rectangle ([yshift=-3in]current page.north east);
...
\begin{tikzpicture}[remember picture, overlay]
  \draw[rounded corners=12pt, fill=rhcard, draw=rhborder, thick]
    ([xshift=0.8in, yshift=-6.0in]current page.north west)
    rectangle ++(34.4in, -16.5in);
\end{tikzpicture}
...
\begin{tikzpicture}
  \draw[rounded corners=6pt, fill=rhcard, draw=rhgreen, thick]
    (0,0) rectangle (\columnwidth, 3.5in);
  \node[anchor=north west, text=rhtext, inner sep=0.15in]
    at (0,3.5in) {\parbox{\dimexpr\columnwidth-0.3in\relax}{
...
\begin{tikzpicture}
  \draw[rounded corners=6pt, fill=rhcard, draw=rhgreen, thick]
    (0,0) rectangle (\columnwidth, 4.2in);
  \node[anchor=north west, text=rhtext, inner sep=0.15in]
    at (0,4.2in) {\parbox{\dimexpr\columnwidth-0.3in\relax}{
      \renewcommand{\arraystretch}{1.3}
      \begin{tabular}{lccc}
      \toprule
...
\begin{tikzpicture}
  \draw[rounded corners=6pt, fill=rhcard, draw=rhgreen, thick]
    (0,0) rectangle (\columnwidth, 2.2in);
  \node[anchor=north west, text=rhgreen, font=\bfseries\LARGE]
    at (0.2in, 2.0in) {Key Takeaways};
  \node[anchor=north west, text=rhtext, font=\large]
    at (0.2in, 1.5in) {$\bullet$ \textbf{Monthly:} No model beats always-long (62.5\%)};
  \node[anchor=north west, text=rhtext, font=\large]
    at (0.2in, 1.1in) {$\bullet$ \textbf{3-month:} CatBoost \& Ensemble beat baseline};
  \node[anchor=north west, text=rhtext, font=\large]
    at (0.2in, 0.7in) {$\bullet$ No model predicts \textit{how much} market moves};
  \node[anchor=north west, text=rhtext, font=\large]
    at (0.2in, 0.3in) {$\bullet$ Signal exists at quarterly horizon, not monthly};
\end{tikzpicture}
